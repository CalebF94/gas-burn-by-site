...
# create gas_day, same logic as generation
...
#possible replacement for the following cell
...
def load_generation_pgs():
    ...
    SUM(he24) AS he24
FROM dbo.loadshapeprofile
WHERE loadshape LIKE '%PGS%'
  AND loadshape LIKE '%- Net Generation-5m'
  AND begtime >= DATEADD(year, -2, GETDATE())
  AND begtime <= GETDATE()
GROUP BY
    begtime,
    loadshape
...
df = pd.concat([df_nonpgs, df_pgs], ignore_index=True)




print("\nGENERATION DATE RANGE")
print(df["begtime"].min())
print(df["begtime"].max())

future_rows = df[
    pd.to_datetime(df["begtime"]) > "2030-01-01"
]
...
print("\nFuture row count:")
print(len(future_rows))



# convert date using begtime
df["begtime"] = pd.to_datetime(df["begtime"])
df["date"] = df["begtime"].dt.date


# map site
df["site"] = "N/A"
df.loc[df["loadshape"].str.contains("DCS"), "site"] = "DCS"
df.loc[df["loadshape"].str.contains("LCS"), "site"] = "LCS"
df.loc[df["loadshape"].str.contains("PGS"), "site"] = "PGS"
df.loc[df["loadshape"].str.contains("GGS"), "site"] = "GGS"
df.loc[df["loadshape"].str.contains("CULBERTSON"), "site"] = "CGS"
...
hourly_df = df.melt(id_vars=["begtime", "site"], value_vars=hour_cols, var_name="hour", value_name="hourly_mw")

#cell 2


# remove impossible generation spikes
hourly_df.loc[hourly_df["hourly_mw"] > 2000, "hourly_mw"] = np.nan
...
# build datetime
hourly_df["datetime"] = (pd.to_datetime(hourly_df["begtime"]) + pd.to_timedelta(hourly_df["hour_num"] - 1, unit="h"))
...
print("\nSITE COUNTS")
print(
    hourly_df.groupby("site").size()
)

print("\nUNIQUE DAYS")
print(hourly_df["datetime"].dt.date.nunique())
...
print("Unique gas days:")
print(hourly_df["gas_day"].nunique())

print("Gas day min:")
print(hourly_df["gas_day"].min())

print("Gas day max:")
print(hourly_df["gas_day"].max())
...
print("Rows before grid:", len(hourly_df))

tmp = (hourly_df.set_index(["gas_day","site","hour"]).reindex(full_grid).reset_index())

print("Rows after grid:", len(tmp))

print(tmp["hourly_mw"].isna().mean())
...
# apply grid
hourly_df = (hourly_df.set_index(["gas_day", "site", "hour"]).reindex(full_grid).reset_index())

# flag missing MW BEFORE any fill
hourly_df["mw_missing_flag"] = hourly_df["hourly_mw"].isna()
...
#daily total by site
daily_site_gen = (hourly_df.groupby(["gas_day", "site"], as_index=False)["hourly_mw"].sum().rename(columns={"hourly_mw": "daily_site_gen_mw"}))
...
print("\n===== GENERATION BY SITE =====")
print(daily_site_gen.groupby("site")["daily_site_gen_mw"].describe())
...
check_df = (gas_daily_site.merge(daily_site_gen, on=["gas_day","site"], how="inner"))
pgs = check_df[check_df["site"] == "PGS"]





check_df["heat_rate"] = (check_df["daily_gas_burn"] / check_df["daily_site_gen_mw"].replace(0, np.nan))
pgs_bad = check_df[(check_df["site"] == "PGS") & (check_df["daily_site_gen_mw"] == 0) & (check_df["daily_gas_burn"] > 0)]

print(check_df.groupby("site")["heat_rate"].describe())



pgs_check = (check_df[check_df["site"] == "PGS"]
    [["gas_day", "daily_gas_burn", "daily_site_gen_mw", "heat_rate"]])

print(pgs_check.sort_values( "daily_gas_burn", ascending=False).head(50))
...
# merge df
merged = hourly_df.merge(daily_site_gen, on=["gas_day", "site"],  how="left")
...
sns.boxplot(data=plot_df, x="site", y="gas_per_mw", order=site_order, showfliers=True)

sns.stripplot( data=plot_df, x="site", y="gas_per_mw", order=site_order, color="black", alpha=0.2, size=2)

plt.ylim(5, 20)
plt.title("Hourly Gas Burn per MWh by Site")
plt.ylabel("MMBtu per MWh")
plt.xlabel("Site")
...
# sns.boxplot(data=clean_df, x="site", y="gas_per_mw")
# plt.show()
...
export_df["bad_flag"] = ((export_df["daily_site_gen_mw"] < 200) | (export_df["gas_per_mw"] > 20) | (export_df["gas_per_mw"] < 5))
...
excel_files = [
    r"G:\Trading\Market Operations\Unit availability\2026\RT Unit availability 06.01.xlsx",
    r"G:\Trading\Market Operations\Unit availability\2026\RT Unit availability 05.01.xlsx",
    r"G:\Trading\Market Operations\Unit availability\2026\RT Unit availability 04.01.xlsx",
    r"G:\Trading\Market Operations\Unit availability\2026\RT Unit availability 03.02.xlsx",
    r"G:\Trading\Market Operations\Unit availability\2026\RT Unit availability 02.05.xlsx",
...
print(result_df["datetime"].min())
print(result_df["datetime"].max())
...
#filtered version for analysis getting rid of the dates from 2040
...
print(master_df[["datetime", "site", "hourly_mw", "availability_mw", "utilization"]].head(20))

calc_util = (master_df["hourly_mw"] / master_df["availability_mw"])

print("\nStored Utilization:")
print(master_df["utilization"].head(10).tolist())

print("\nCalculated Utilization:")
print(calc_util.head(10).tolist())
...
master_df.loc[ master_df["availability_mw"] < 10, "utilization"] = np.nan
...
#Cell 19
#cleaning the df for utilization
master_df.loc[master_df["utilization"] > 1.2, "utilization"] = np.nan
...
print(site_df.head(20))

print("\nRows:", len(site_df))

print("\nSites:")
print(site_df["site"].unique())
...
#call the API
df_forecast_raw = pull_yes_forecast_historical( YES_USER, YES_PASS, start_date="2024-06-01", end_date="2026-06-01")
...
def clean_yes_actual(df):
    ...
    df["wind_actual"] = df[wind_cols].apply(pd.to_numeric, errors="coerce").sum(axis=1)
    ...
    df["temperature_actual"] = df[temp_cols].apply(pd.to_numeric, errors="coerce").mean(axis=1)
...
#call teh API
df_actual_raw = pull_yes_actual_historical( YES_USER, YES_PASS, start_date="2024-06-01", end_date="2026-06-01")
...
#data quality flags
master_df["mw_missing_flag"] = master_df["hourly_mw"].isna()
master_df["gas_missing_flag"] = master_df["daily_gas_burn"].isna()

master_df["data_quality_flag"] = (master_df["mw_missing_flag"] | master_df["gas_missing_flag"])
...
master_export = master_df.copy()

master_export = master_export.sort_values(
    ["site", "datetime"]
)



###Final training dataset for XGBoost model
# Export full merged dataset before model training
...
export_master = master_df[[c for c in export_cols if c in master_df.columns]]

export_master.to_csv(r"G:\Trading\Forecasts\Daily Gas Burn Forecast by Site\Training Dataset1 (7-09).csv", index=False)


file_path = r"G:\Trading\Forecasts\Daily Gas Burn Forecast by Site\Training Dataset1 (7-09).xlsx"

export_master.to_excel(file_path, index=False)

print("Master dataset exported:")
print(file_path)
print("Rows:", len(export_master))
print("Columns:", len(export_master.columns))
...
master_export.to_csv( r"G:\Trading\Forecasts\Daily Gas Burn Forecast by Site\MASTER_MERGED_DATA.csv", index=False)
...
# ##################################### ASK  ABOUT  THIS #################################
#     # skip site with not a lot of data
#     if len(train) == 0 or len(test) == 0:
#         print(f"Skipping {site} due to insufficient data")
#         continue
...
# #adding in a flag to highlight bad operating conditions to see if the numbers seem logical
# forecast_df["efficiency_flag"] = ((forecast_df["gas_per_mw"] > 20) | (forecast_df["gas_per_mw"] < 5))


# # final formatting here
# forecast_df = forecast_df[["datetime", "gas_day", "site", "predicted_gas_burn", "gas_per_mw", "implied_mw", "efficiency_flag"]]
...
# #export to excel
# file_path = r"G:\Trading\Forecasts\Daily Gas Burn Forecast by Site\forecast with pgs tests1.xlsx"
